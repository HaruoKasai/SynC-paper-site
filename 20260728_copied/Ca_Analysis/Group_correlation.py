...
# Statistics: all reported comparisons use an exact paired permutation test.
# For each matched unit, the two condition labels are independently swapped;
# all 2^n possible swap patterns are enumerated (no random resampling).
...
def add_friedman_stats(
    ...
    """
    Legacy omnibus-test hook.  No Friedman result is written: this script
    reports only exact paired permutation comparisons below.
...
def _safe_exact_paired_permutation(x, y):
    """Exact two-sided paired permutation test using the mean difference.

    For n matched observations, this enumerates all 2**n sign flips of
    (y - x).  It is therefore exact, rather than a Monte-Carlo estimate.
    """
    ...
    # Rows are sign-flipped independently under the paired null hypothesis.
    signs = np.asarray(list(product((-1.0, 1.0), repeat=len(diff))), dtype=float)
    permuted = (signs @ diff) / len(diff)
    # Numerical tolerance prevents equivalent floating-point values from
    # being mistakenly excluded at the boundary.
    extreme = np.count_nonzero(np.abs(permuted) >= abs(observed) - 1e-12)
    p = extreme / len(permuted)
    return observed, float(p), int(len(x)), "exact 2^n paired label-swap permutations"


def add_exact_permutation_stats(stats_records, df, metric_name, value_col, unit_cols, events, tw_pairs=((0, 1), (1, 2), (0, 2))):
    """
    ...
    if any(c not in df.columns for c in required):
        stats_records.append({
            "test_family": "exact_paired_permutation",
            "metric": metric_name,
            "event_name": "ALL",
            "comparison": "NA",
            "n": 0,
    ...
    for ev in events:
        ...
        for a, b in tw_pairs:
            ...
            stat, p, n, note = _safe_exact_paired_permutation(paired[a], paired[b])
            ...
            stats_records.append({
                ...
                "p_value": p,
                "effect": effect,
                "model": "exact paired label-swap permutation; statistic=mean(tw_b - tw_a); unit=" + "+".join(unit_cols),
                "note": note
            })
...
def add_cross_event_exact_permutation_stats(
    stats_records,
    df,
    metric_name,
    value_col,
    ...
    """Exact paired permutation test between two events at one time window."""
    ...
    if (event_a not in pivot.columns) or (event_b not in pivot.columns):
        ...
        stat, p, n, note = _safe_exact_paired_permutation(paired[event_a], paired[event_b])
    ...
    stats_records.append({
        ...
        "p_value": p,
        "effect": effect,
        "model": "exact paired label-swap permutation; statistic=mean(event_b - event_a); unit=" + "+".join(unit_cols),
        "note": note
    })
...
def process_group(path, window_len, n_windows):
    ...
    for name, analysis_conditions in analysis_condition_sets.items():
        for seed in [1]: #range(2,10)
            ...
            stats_records = add_exact_permutation_stats(
                stats_records,
                df_mouse_corr_F,
                metric_name="mouse_all_cell_spearman_pairwise_correlation",
                value_col="r_mouse_mean_spearman",
            ...
            for cross_df, cross_metric, cross_value, cross_units in cross_event_metrics:
                stats_records = add_cross_event_exact_permutation_stats(
                    stats_records,
                    cross_df,
