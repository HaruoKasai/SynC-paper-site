...
def process(data_folder):
    ...
    *_, contime = extract_params(data_folder)
    ...
    # before_frames = frame2p_df[frame2p_df['time'] > 0]['frame'].values.tolist() #temp
    ...
    dist = 1.0 - corr_matrix_before
    Z = linkage(squareform(dist, checks=False), method='average')  # 'average' or 'complete'
    global_order = leaves_list(Z)  # デンドログラムの葉順序
    df_sorted = pd.DataFrame({
        "original_cell_index": np.arange(len(global_order)),
        "sort_order": np.argsort(global_order)  # 参考列
    })
    df_sorted.to_csv(os.path.join(data_folder, "_GCaMP", "hier_order.csv"), index=False)

    spks = spks[global_order]
    Fc = Fc[global_order]
    ...
    time_window_sets = {
        ...
        # "20-40min":[[20, 40],[20,25],[30,35],[40,45], [45,50], [50,55]]
    }
    window_len = 240
    n_windows = 7
    seed = 0
    ...
    # n_used_cells = (n_cells_all // pca_batchsize) * pca_batchsize
    n_groups_cell = max(1, n_cells_all // pca_batchsize)
    # n_groups_cell = max(1, n_used_cells // pca_batchsize)
    # used_idx = np.arange(n_used_cells)
    # Fc = Fc[used_idx,:]
        ...
        for name, analysis_time_window in time_window_sets.items():
            ...
            for tw_id, tw in enumerate(analysis_time_window):
                ...
                for event_name, group in event_df_tw.groupby('event_name'):
                    ...
                    # batch_ids = (used_idx // pca_batchsize) + 1
                    ...
                    r_mean_dict[(event_idx, tw_id)] = r_mean_vec
                    ...
                    if len(analysis_time_window) < 12:
                        ...
                        colors = [
                            ...
                            (0.5, '#fffff0'),  # 中心(ほぼ白)
                            (0.62, '#fff5b1'),  # 淡い黄色
                            (0.69, '#ffd166'),  # 明るい黄〜オレンジ
                            (0.74, '#fdae61'),  # オレンジ
                            (0.79, '#f46d43'),  # 朱色（オレンジ→赤の橋渡し）
                            (0.85, '#d73027'),  # 赤
                            (1.00, '#4d0000'),  # 最濃赤
                        ]
                        cmap = LinearSegmentedColormap.from_list("div_multi", colors, N=1024)
                        ...
                        #     mask = batch_ids == g
                        #     r_batch_mat = r_mean_mat[np.ix_(mask, mask)]  # バッチ内セルのみ抽出
                        #
                        #     if r_batch_mat.shape[0] < 2:
                        #         continue  # 1セルしかない場合はスキップ
                        #
                        #     batch_png_path = os.path.join(
                        #         data_folder,
                        #         "_GCaMP",
                        #         f"_{data_pattern}_correlation_{name}_batch{g}_{str(v)}_{str(event_idx)}_{str(tw_id)}.png"
                        #     )
                        #
                        #     fig_b = plt.figure(figsize=(4, 4))
                        #     ax_b = fig_b.add_axes([0, 0, 1, 1])
                        #     sns.heatmap(r_batch_mat, cmap=cmap, vmin=-v, vmax=v, square=True,
                        #                 ax=ax_b, xticklabels=False, yticklabels=False, cbar=False)
                        #     ax_b.set_axis_off()
                        #     fig_b.savefig(batch_png_path, dpi=300, bbox_inches='tight', pad_inches=0)
                        #     plt.close(fig_b)
            ...
            # r_meanのscatter plot
            save_dir = os.path.join(data_folder, "_GCaMP","scatter_plots")
            os.makedirs(save_dir, exist_ok=True)
            keys = list(r_mean_dict.keys())
            for key_x, key_y in itertools.combinations(keys, 2):
                x = r_mean_dict[key_x]
                y = r_mean_dict[key_y]
                ...
                ex, tx = key_x
                ey, ty = key_y
                filename = f"scatter_e{ex}_t{tx}_vs_e{ey}_t{ty}.pdf"
