...
def process_folder(data_folder, analysis_time_window, data_pattern, records, pca_results,pr_results, seed, pca_batchsize):
    ...
    return records, pca_results, pr_results
...
def process_group (path):
    ...
    for name, analysis_time_window in time_window_sets.items():
        for seed in range(1):
            ...
            fig = plt.figure(figsize=(len(analysis_time_window) * 4, 15))
            gs = gridspec.GridSpec(4, 1)
            plt.subplots_adjust(wspace=0.05, hspace=0.05)
            ax1, ax2, ax3, ax4= fig.add_subplot(gs[0, 0]), fig.add_subplot(gs[1, 0]), fig.add_subplot(gs[2, 0]),fig.add_subplot(gs[3, 0])
            records_F = []
            records_spks = []
            pca_results_F = []
            pr_results_F = []
            ...
            for mouse in mouse_list:
                records_F, pca_results_F, pr_results_F= process_folder (mouse, analysis_time_window, "F", records_F, pca_results_F,pr_results_F, seed, pca_batchsize=20)
