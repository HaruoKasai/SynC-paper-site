...
def CID_timeseries(spine_csv, back_csv, den_csv, tp_base=[0,8], color_list=["forestgreen", "orangered","forestgreen"], interval=20, rapa_wash_min = 10): #
    ...
    den_df = pd.read_csv(den_csv) # zhou, dendrites means
    area = spine_df['area_in_pixel'].values.reshape(-1,1)
    sp_minus_back_x_area = (spine_df.iloc[:, 2:] - back_df.iloc[:, 2:])*area
    den_minus_back_x_area = (den_df.iloc[:, 2:] - back_df.iloc[:, 2:])*area # zhou, (dendrites means - back means)*spine area
    print("sp_minus_back_x_area")
    print(sp_minus_back_x_area)
    print("den_minus_back_x_area")
    print(den_minus_back_x_area)
    tp = int(sp_minus_back_x_area.shape[1]/2) #2 color用
    calc = ["AU","delta(%)"]
    fig, axes = plt.subplots(nrows=len(color_list)+1, ncols=2, figsize=(6, len(color_list)*3)) # zhou, Fig arrangement, add 1 rows
    col_list = []
    for i in range(tp):
    ...
    for c in range(len(color_list)):
        ...
        for cal in range(len(calc)):
            ...
            # Zhou, Calculation mVenus(syn)= mVenus(spine) - mScarlet(spine) * (mVenus(dendrites)/mScarlet(dendrites))
            calc_df = (
                    sp_minus_back_x_area.iloc[:, :tp].set_axis(col_list, axis="columns")
                    - sp_minus_back_x_area.iloc[:, tp:2 * tp].set_axis(col_list, axis="columns")
                    * (den_minus_back_x_area.iloc[:, :tp].set_axis(col_list, axis="columns")
                       / den_minus_back_x_area.iloc[:, tp:2 * tp].set_axis(col_list, axis="columns"))
            )
            calc_df = calc_df.iloc[:, :tp]
            print(calc_df)

            average = calc_df.mean(axis='index').values.reshape(-1, 1)
            sem = calc_df.sem(axis='index').values.reshape(-1, 1)
            print("calc_df.shape =", calc_df.shape)
            print("tp_list.shape =", tp_list.shape)
            calc_T = calc_df.T
            calc_T["min"] = tp_list

            ax = calc_T.plot(x="min", ax=axes[len(color_list), 0], legend=False, linewidth=0.5, cmap="bone")
            ax.set_ylabel("mVenus(synapse)")
            ax.axvspan(0, rapa_wash_min, color="blue", alpha=0.3, linewidth=0)
            ax.spines['right'].set_visible(False)
            ax.spines['top'].set_visible(False)
            ax.axhline(y=0, color="gray", ls="dotted", lw=1)
            ...
            # 保存CSV
            calc_T["average"] = average
            calc_T["sem"] = sem
            calc_T.to_csv(file_name + "_calc_AU.csv")

            # 均值 ± SEM
            ax.plot(tp_list, average, color="yellowgreen", linewidth=1.5)
            x = tp_list.reshape(tp_list.shape[0])
            y1 = (average - sem).reshape(average.shape[0])
            y2 = (average + sem).reshape(average.shape[0])
            ax.fill_between(x, y1, y2, color="yellowgreen", alpha=0.3, linewidth=0)
...
for spine_csv in spine_csv_files:
    ...
    den_csv = spine_csv[:-9]+"den.csv"
    CID_timeseries(spine_csv, back_csv, den_csv)
