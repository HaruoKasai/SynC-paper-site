...
def delta_data_collection(dir, color_num=3, channel_list=["c0", "c1", "c2"],  tp=2): #c2: mVenus/mScarlet ratio
    cond_list = glob.glob(os.path.join(dir, "[!_]*"))
    print("number of conditions: %s" % len(cond_list))
    fig, axes = plt.subplots(nrows=4, ncols=1, figsize=(3, 3* 3))
    ...
    #zhou, mVenus(synapse calculation)
        df_calc = pd.DataFrame(index=[])
        for date in date_list:
            csv_list = glob.glob(os.path.join(date, "graph", "*_calc_AU.csv"))
            for csv in csv_list:
                tp = pd.read_csv(csv).loc[:, ["min"]]
                df_ind = pd.read_csv(csv).iloc[:, 1:-3]
                df_calc = pd.concat([df_calc, df_ind], axis=1)
        if not df_calc.empty:
            average = df_calc.mean(axis="columns").values.reshape(-1, 1)
            sem = df_calc.sem(axis="columns").values.reshape(-1, 1)
            df_calc = pd.concat([df_calc, tp], axis=1)
            df_calc["average"] = average
            df_calc["sem"] = sem
            df_calc.to_csv(os.path.join(dir, "_summary", os.path.basename(cond) + "_calc_AU.csv"))

            ax = df_calc.plot(x="min", y="average", yerr="sem", elinewidth=0.6,
                              ax=axes[color_num], legend=False, linewidth=1.2,
                              color=color_list[cond_list.index(cond)])
            ax.axvspan(0, 2, color="lightblue", alpha=0.3, linewidth=0)
            ax.spines['right'].set_visible(False)
            ax.spines['top'].set_visible(False)
            ax.axhline(y=0, color="gray", ls="dotted", lw=1)
...
root = tkinter.Tk()
root.withdraw()
dir = tkinter.filedialog.askdirectory(initialdir=r"\\Synology\zhou")
