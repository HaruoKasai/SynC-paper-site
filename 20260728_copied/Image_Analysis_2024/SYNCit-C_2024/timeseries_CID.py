...
# def CID_timeseries(spine_csv, back_csv, tp_base=[0, 3], color_list=["forestgreen", "orangered", "forestgreen"],interval=60, rapa_wash_min=10): #PSD95
    ...
    cnum = sum([1 for item in spine_df.columns.tolist() if item.startswith('mean0')])
    tp = int(sp_minus_back_x_area.shape[1]/cnum)
    calc = ["AU", "delta(%)"]
    ...
    if cnum > 1:
        ...
        df_concat = sp_minus_back_x_area
    for c in range(cnum):
