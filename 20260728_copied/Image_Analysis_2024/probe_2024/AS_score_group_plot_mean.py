...
def calculate_index(input_df, col_name, probe_name, index_df, index_type="hot_spot", c_fit=None, ave=None):
    ...
    for cell in cell_list:
        ...
        for dend in dend_list:
            ...
            if index_type == "hot_spot":
            ...
            elif index_type == "percentage_above_2SD":
                mean = ave
                std = c_fit
                count = ((df_dend[col_name]) > (mean + 2 * std)).sum()
                ...
                print(mean)
                print(std)
group1_probes = {16, 173, 174}
group2_probes = {176, 177, 213, 217}
def plot_grouped_graph (df,probe_group, position, title):
    ax = fig.add_subplot(gs[(position)])
    df['probe'] = df['probe'].astype(str)
    probe_group = {str(p) for p in probe_group}
    df_filtered = df[df['probe'].isin(probe_group)]
    print("Filtered DataFrame:")
    print(df_filtered)
    if df_filtered.empty:
        print(f"Warning: No data found for probes {probe_group} in df['probe'].")
        print(f"Available probes: {df['probe'].unique()}")
    sns.barplot(x='probe', y='index_val', data=df_filtered, ax=ax)
    sns.stripplot(x='probe', y='index_val', data=df_filtered, ax=ax, color='black', alpha=0.6, size=2, jitter=True)
    ...
    if position == (2,2):
        ax.set_ylim(0,50)
        ax.set_ylabel("percent of spine(%)")
    elif position == (2,3):
        ax.set_ylim(0, 50)
        ax.set_ylabel("percent of spine(%)")
    # if position == (0,10):
...
df_all.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_all_data.csv", index=False)
df_cell.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_cell_data.csv", index=False)
...
for p, probe in enumerate(probe_list):
    ...
    # for i, cell in enumerate(unique_cells):
    #     cell_data = df_probe[df_probe['cell'] == cell]
    #     ax.scatter(cell_data['V_normalized'], cell_data['AS/filler'],  s=5, alpha=0.5,
    #                label='Control')
    #     #ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: f'{x / 1000:.1f}k'))
    #     ax.set_xlabel('V (a.u.)')
    ...
    # for i, cell in enumerate(unique_cells):
    #     cell_data = df_probe[df_probe['cell'] == cell]
    #     ax.scatter(cell_data['V_normalized'], cell_data['AS/filler_normalized'], s=5, alpha=0.5,
    #                label='Control')
    #     # ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: f'{x / 1000:.1f}k'))
    #     ax.set_xlabel('V (a.u.)')
    ...
    for i, cell in enumerate(unique_cells):
        ...
        # ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: f'{x / 1000:.1f}k'))
        ax.set_xlabel('V (a.u.)')
        ax.set_ylabel(f'AS ratio (filler norm) {os.path.basename(probe)}')
        ax.set_ylim(-0.2, 1.8)
        ax.set_title('Control AS ratio')
        # ax.set_xlim(0, 6)
    ...
    for i, cell in enumerate(unique_cells):
        ...
        # ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda x, pos: f'{x / 1000:.1f}k'))
        ax.set_xlabel('V (a.u.)')
        ax.set_ylabel(f'subtraction_mode_ratio {os.path.basename(probe)}')
        # ax.set_ylim(-1000, 100000)
    ...
    ax.set_title(f"histogram subtraction {os.path.basename(probe)}")
    ax.set_xlabel("subtraction_mode_AS_filler_ratio")
    ax.set_ylabel("frequency")
    ax.set_xlim(-0.2, 1.8)
    # if p == 4:
    #     inset_ax = inset_axes(ax, width='40%', height='40%', loc="upper right")
    #     #inset_ax.set_xlim(4, 30)
    #     #inset_ax.set_ylim(0, 5)
    #     inset_ax.hist(df_probe['subtraction_mode_AS_filler_ratio'], bins=100, alpha=0.7)
    #     inset_counts, inset_bins, inset_patches = inset_ax.hist(df_probe['subtraction_mode_AS_filler_ratio'], bins=100, alpha=0.7)

    ax = fig.add_subplot(gs[(p, 9)])
    #probe毎のsubtraction ratioのmodeを算出
    kde = stats.gaussian_kde(df_probe['subtraction_mode_AS_filler_ratio'])
    x_vals = np.linspace(min(df_probe['subtraction_mode_AS_filler_ratio']), max(df_probe['subtraction_mode_AS_filler_ratio']), 250)
    kde_vals = kde(x_vals)
    mode_value_kde = x_vals[np.argmax(kde_vals)]

    bin_width = 6e-03
    bin_edges_all = np.arange(min(df_probe['subtraction_mode_AS_filler_ratio']),
                              max(df_probe['subtraction_mode_AS_filler_ratio']) + bin_width, bin_width)

    hist_all, bin_edges_all = np.histogram(df_probe['subtraction_mode_AS_filler_ratio'], bins=bin_edges_all, density=True)
    hist_max_all = hist_all.max()  # ヒストグラムの最大値
    hist_normalized_all = hist_all / hist_max_all  # 最大値で正規化
    bin_centers_all = 0.5 * (bin_edges_all[1:] + bin_edges_all[:-1])

    # modeまででgaussian fitを行いc_fit;SDを算出
    filtered_data = df_probe['subtraction_mode_AS_filler_ratio'][df_probe['subtraction_mode_AS_filler_ratio'] <= mode_value_kde]
    ...
    hist, bin_edges = np.histogram(symmetric_data, bins=bin_edges_all, density=True)
    hist_max = hist.max()
    hist_normalized = hist / hist_max
    bin_centers = 0.5 * (bin_edges[1:] + bin_edges[:-1])
    print(max(hist))
    bin_width = np.diff(bin_edges)
    bin_widths = bin_width.mean()
    print(f'bin_width: {bin_widths}')

    popt, _ = curve_fit(gaussian, bin_centers, hist, p0=[max(hist), mode_value_kde , np.std(symmetric_data)], maxfev=1000)
    ...
    x_fit = np.linspace(min(symmetric_data)- 0.1, max(df_probe['subtraction_mode_AS_filler_ratio']), 1000)
    ...
    hist_z0, bin_edges_z0 = np.histogram(symmetric_zscore_data, bins=bin_edges_z0, density=True)
    ...
    x_fit_z0 = np.linspace(min(z_scores) - 3, max(z_scores), 1000)
    ...
    ax.set_title(f"zscore_hist and gaus fit for {os.path.basename(probe)}")
    ...
    #     print(
    #         f"hist_zscores_Bin {i + 1} ({bin_zscores[i]:.2f} to {bin_zscores[i + 1]:.2f}): {hist_zscores[i]} elements")
    # for j in range(len(hist_zscores_above)):
    #     print(
    #         f"hist_zscores_above_Bin {j + 1} ({bin_zscores_above[j]:.2f} to {bin_zscores_above[j + 1]:.2f}): {hist_zscores_above[j]} elements")
...
for probe in df_all['probe'].unique():
    ...
    calculate_index(df, col_name=scoring, probe_name=probe, index_df=percentage_2sd_df, index_type="percentage_above_2SD", c_fit=c_fit_dict[os.path.basename(probe)], ave=mean_subtraction_ratio_dict[os.path.basename(probe)])
...
plot_grouped_graph(hotspot_index_df, group1_probes, position=(0,2), title="Hot spot index")
plot_grouped_graph(hotspot_index_df, group2_probes, position=(0,3), title="Hot spot index")
plot_grouped_graph(SD_df,group1_probes, position=(1,2), title="SD")
plot_grouped_graph(SD_df,group2_probes, position=(1,3), title="SD")
plot_grouped_graph(percentage_2sd_df,group1_probes, position=(2,2), title="Percentage of spines above 2SD")
plot_grouped_graph(percentage_2sd_df,group2_probes, position=(2,3), title="Percentage of spines above 2SD")
plot_grouped_graph(above_2SD_mean_df,group1_probes, position=(3,2), title="above_2SD_mean")
plot_grouped_graph(above_2SD_mean_df,group2_probes, position=(3,3), title="above_2SD_mean")
plot_grouped_graph(zscore_combined_df.rename(columns={'z_score': 'index_val'}),group1_probes, position=(4, 2), title='z-score')
plot_grouped_graph(zscore_combined_df.rename(columns={'z_score': 'index_val'}),group2_probes, position=(4, 3), title='z-score')



# 結果の保存
hotspot_index_df.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_hotspot_index.csv", index=False)
SD_df.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_standard_deviation.csv", index=False)
percentage_2sd_df.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_percentage_above_2SD.csv", index=False)
above_2SD_mean_df.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_above_2SD_mean.csv", index=False)
zscore_combined_df.to_csv(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_z_score.csv", index=False)

# プロットをPDFに保存
plt.tight_layout()
fig.savefig(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data\_summary_NoAPV.pdf", dpi=100, transparent=True)
