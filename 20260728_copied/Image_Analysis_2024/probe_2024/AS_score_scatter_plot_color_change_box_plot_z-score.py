...
def calculate_as_score(spine_csv, back_csv):
    ...
    return sp_minus_back_x_area, as_score  # Return the entire array (not excluding the last row)
...
for probe in probe_list:
    ...
    color_list = sns.color_palette("husl", len(spine_csv_files))  # データセットの数だけ色を用意

    plt.figure(figsize=(8, 6))  # Control群の散布図をまとめるための図を作成

    for idx, (spine_csv, back_csv) in enumerate(zip(spine_csv_files, back_csv_files)):
        if "control" in spine_csv.lower():
            ...
            # Control のデータを1つの散布図にまとめる
            plt.scatter(as_X, as_scores, alpha=0.5, color=color_list[idx], label=os.path.basename(spine_csv))

    # Scatter plot settings for Control
    plt.xlabel('filler_sum (a.u)')
    plt.ylabel('AS Score')
    plt.ylim(0, 35)  # 必要に応じて Y 軸の範囲を調整
    plt.title('Control AS Score Scatter Plot')
    plt.legend()
    plt.savefig(os.path.join(dir, 'control_combined_as_scatter_plot.pdf'), dpi=300, transparent=True)
    plt.close()

    plt.figure(figsize=(8, 6))  # APV群の散布図をまとめるための図を作成

    for idx, (spine_csv, back_csv) in enumerate(zip(spine_csv_files, back_csv_files)):
        if "apv" in spine_csv.lower():
            as_X, as_scores = calculate_as_score(spine_csv, back_csv)
            apv_as_X.extend(as_X)
            apv_as_Y.extend(as_scores)
            apv_as_scores.extend(as_scores)

            # APV のデータを1つの散布図にまとめる
            plt.scatter(as_X, as_scores, alpha=0.5, color=color_list[idx], label=os.path.basename(spine_csv))

    # Scatter plot settings for APV
    plt.xlabel('filler_sum (a.u)')
    plt.ylabel('AS Score')
    plt.ylim(0, 35)  # 必要に応じて Y 軸の範囲を調整
    plt.title('APV AS Score Scatter Plot')
    plt.legend()
    plt.savefig(os.path.join(dir, 'apv_combined_as_scatter_plot.pdf'), dpi=300, transparent=True)
    plt.close()
