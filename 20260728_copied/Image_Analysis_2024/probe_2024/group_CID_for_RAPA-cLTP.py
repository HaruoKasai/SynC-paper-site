...
def delta_data_collection(dir, color_num=3, channel_list=["c0", "c1", "c2"],  tp=2): #c2: mVenus/mScarlet ratio
    ...
    for cond in cond_list:
        ...
        for c in range(color_num):
            ...
            ax = df.plot(x="min", y="average", yerr="sem", elinewidth=0.6 ,ax=axes[c], legend=False, linewidth=1.2,color=color_list[cond_list.index(cond)], label=os.path.basename(cond))
            ax.axvspan(-3, 7, color="lightblue", alpha=0.3, linewidth=0)
            ax.axvspan(60, 75, color="orange", alpha=0.3, linewidth=0)
            ...
            ax.set_xlim(-50, 160)
            ax.set_xticks(np.arange(-50, 160, 50))
            ax.set_ylim(-50, 60)
            ax.set_yticks(np.arange(-50, 60, 25))
