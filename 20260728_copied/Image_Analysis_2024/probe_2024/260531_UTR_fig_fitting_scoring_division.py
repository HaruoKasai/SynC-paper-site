...
CENTERED_SCORE_COL = "AS_filler_ratio_filler_normalized_probe_median_divided"
...
def add_probe_median_division(df_all):
    probe_median_values = (
        df_all.groupby("probe")[RAW_SCORE_COL]
        .transform("median")
        .replace(0, np.nan)
    )
    df_all[CENTERED_SCORE_COL] = df_all[RAW_SCORE_COL] / probe_median_values
    return df_all
...
def fit_half_gaussian(data, bin_width=6e-03):
    ...
    return {
        "method": "half_gaussian_global_median_division",
...
def fit_gmm_base(data, max_components=4, min_component_weight=0.05):
    ...
    return {
        "method": "gmm_base_global_median_division",
...
def plot_summary_pdf(df_scored, summary_df, fit_result, z_col, output_root):
    ...
    for p, probe in enumerate(probes):
        ...
        ax.set_title("Probe median-divided")
        ...
        ax.set_xlabel("median-divided score")
...
def main():
    ...
    df_all = add_probe_median_division(df_all)

    df_all.to_csv(os.path.join(OUTPUT_ROOT, "_all_data_before_global_scoring_median_division.csv"), index=False)
    df_cell.to_csv(os.path.join(OUTPUT_ROOT, "_cell_data_median_division.csv"), index=False)
