...
def Individual_timeseries(spine_csv, back_csv, param_json, color_list=["forestgreen", "orangered","forestgreen"]):
    ...
    shade_min = params['shade_min']
    ms = params['ms']
    spine_dict = params["spine_dict"]
    #     # print(spine_dict)
    ...
    output_dir = os.path.join(dir, "timeseries_ind")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    for key, value in spine_dict.items():
        spine_dir = os.path.join(output_dir, key)
    ...
    for c in range(len(color_list)):
        ...
        for cal in range(len(calc)):
            ...
            for key, value in spine_dict.items():
                ...
                spine_dir = os.path.join(output_dir, key)
                file_name = os.path.join(spine_dir, fname[:-9])
                df_sp = df_T.iloc[:, value[0]:value[1]]
                df_sp.columns = [str(i+1) for i in range(len(df_sp.columns))]
                print(df_sp)

                if df_sp.shape[1]>1:
                    average = df_sp.mean(axis="columns").values
                    sem = df_sp.sem(axis='columns').values
                df_sp["min"] = tp_list
                print(df_T)
                ax = df_sp.plot(x="min", ax=axes[c, cal], legend=False, linewidth=0.5, cmap="bone")
                ax.set_ylabel(calc[cal])
                ax.axvspan(0,shade_min,color = "blue", alpha=0.3, linewidth=0)
                ...
                df_sp.to_csv(file_name + "c" + str(c) + "_" + calc[cal] + ".csv")
                ax.set_xlim(-144, 496)
    ...
    fig.savefig(file_name + "timeseries.pdf", dpi=300,  transparent=True) #TODO
...
spine_csv_files = glob.glob(os.path.join(dir,"csv", "*spine.csv"))
param_json = os.path.join(os.path.dirname(os.path.dirname(dir)), "_param.json")
print("number of files: %s" % len(spine_csv_files))

for spine_csv in spine_csv_files:
    back_csv = spine_csv[:-9]+"back.csv"
    Individual_timeseries(spine_csv, back_csv, param_json)
