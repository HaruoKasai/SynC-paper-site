...
def binning_image(image, bin_size=2):
    """画像を指定されたビンサイズで平均化して縮小"""
    # 画像サイズをビンサイズで割り切れるようにトリミング
    trimmed_height = (image.shape[0] // bin_size) * bin_size
    trimmed_width = (image.shape[1] // bin_size) * bin_size
    trimmed_image = image[:trimmed_height, :trimmed_width]

    # ビニング処理
    new_shape = (trimmed_height // bin_size, bin_size, trimmed_width // bin_size, bin_size)
    binned_image = trimmed_image.reshape(new_shape).mean(axis=(1, 3))
    return binned_image
...
# 背景をしきい値で除去したマスク
background_threshold = 5  # 背景と見なす閾値（調整可能）
...
dilated_binary_mask = binary_dilation(binary_mask_channel2, structure=np.ones((35, 35)))

# バイナリダイレーションを適用したマスクを元のデータに適用
dilated_safe_image_channel2 = np.where(dilated_binary_mask, image_channel2, np.nan)

bin_size = 100  # ビニングサイズ（2x2の平均化）
binned_dilated_safe_image_channel2 = binning_image(dilated_safe_image_channel2, bin_size)
binned_safe_image_channel1 = binning_image(safe_image_channel1, bin_size)
...
ratio_image = np.clip(ratio_image, 0, np.nanpercentile(ratio_image, 98.0))
...
# カラーマップの指定
colors = [(0.0, "purple"), (0.2, "blue"), (0.4, "skyblue"), (0.6, "green"), (0.8, "yellow"), (1.0, "red")] # (0.8, "orange"),
...
fname = os.path.splitext(os.path.basename(channel1_path))[0]
output_fname = f"{fname}_ratio_mapping_with_dilation.pdf"
output_path = os.path.join(os.path.dirname(channel1_path), output_fname)

# 画像の保存
fig.savefig(output_path, dpi=300, facecolor='black', bbox_inches='tight', transparent=False)
plt.show()
