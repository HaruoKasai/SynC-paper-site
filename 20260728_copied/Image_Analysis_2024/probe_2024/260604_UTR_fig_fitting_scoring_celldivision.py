...
INPUT_ROOT = r"\\Synology\arima\Probe_paper_2023\dissociate\distribution"
OUTPUT_ROOT = INPUT_ROOT

RAW_SCORE_COL = "AS/filler_ratio_filler_normalized"
CENTERED_SCORE_COL = "AS_filler_ratio_filler_normalized_cell_median_divided"
...
def add_cell_median_division(df_all):
    """Normalize each cell by dividing the filler-normalized ratio by that cell median."""
    cell_median_values = (
        df_all.groupby(["probe", "cell"])[RAW_SCORE_COL]
        .transform("median")
        .replace(0, np.nan)
    )
    df_all["cell_median_AS_filler_ratio_filler_normalized"] = cell_median_values
    df_all[CENTERED_SCORE_COL] = df_all[RAW_SCORE_COL] / cell_median_values
    return df_all
...
def fit_half_gaussian(data, bin_width=6e-03):
    ...
    return {
        "method": "half_gaussian_global_cell_median_division",
...
def fit_gmm_base(data, max_components=4, min_component_weight=0.05):
    ...
    return {
        "method": "gmm_base_global_cell_median_division",
...
def save_nonparametric_tests(summary_df, method, output_root):
    ...
    top25_hotspot_df = select_top_fraction_by_probe(summary_df, "hotspot_index", fraction=0.25)
    if not top25_hotspot_df.empty:
        metric = "top25_hotspot_index"
        test_df = top25_hotspot_df[["probe", "hotspot_index"]].dropna()
        grouped_values = [
            group["hotspot_index"].values
            for _, group in test_df.groupby("probe")
            if len(group["hotspot_index"].dropna()) > 0
        ]
        ...
        top25_for_dunn = top25_hotspot_df.rename(columns={"hotspot_index": metric})
        dunn_df = dunn_test_with_holm(top25_for_dunn, metric)
        if not dunn_df.empty:
            dunn_df.insert(0, "method", method)
            dunn_rows.append(dunn_df)
...
def save_method_outputs(df_all, fit_result, output_root):
    ...
    top25_hotspot_df = select_top_fraction_by_probe(summary_df, "hotspot_index", fraction=0.25)
    if not top25_hotspot_df.empty:
        top25_hotspot_df[["hotspot_index", "probe", "cell", "dendrite"]].rename(
            columns={"hotspot_index": "index_val"}
        ).to_csv(
            os.path.join(output_root, f"_top25_hotspot_index_{method}.csv"),
            index=False,
        )
...
def get_random_gaussian_reference_value(metric):
    """Return the expected metric value for independent standard-normal z-scores."""
    if metric == "hotspot_index":
        return 2 / np.sqrt(np.pi)
    if metric == "percentage_above_2SD":
        return stats.norm.sf(2) * 100
    if metric == "percentage_above_3SD":
        return stats.norm.sf(3) * 100
    if metric in ["above_2SD_mean", "mean_above_2SD_z_score"]:
        return stats.norm.pdf(2) / stats.norm.sf(2)
    if metric == "above_3SD_mean":
        return stats.norm.pdf(3) / stats.norm.sf(3)
    if metric in ["mean_z_score", "median_z_score"]:
        return 0
    return None

def select_top_fraction_by_probe(df, metric, fraction=0.25):
    """Select the top fraction of dendrites within each probe for a secondary hotspot-strength view."""
    rows = []
    for _, group in df.dropna(subset=[metric]).groupby("probe"):
        n_top = max(1, int(np.ceil(len(group) * fraction)))
        rows.append(group.sort_values(metric, ascending=False).head(n_top))
...
def plot_metric_bar(fig, gs, df, metric, position, title, ylabel):
    ...
    reference_value = get_random_gaussian_reference_value(metric)
...
def plot_summary_pdf(df_scored, summary_df, fit_result, z_col, output_root):
    ...
    centered_bin_edges = np.linspace(global_x_min, global_x_max, 101)
    centered_bin_width = centered_bin_edges[1] - centered_bin_edges[0]
    ...
    for p, probe in enumerate(probes):
        ...
        #ax.set_ylim(-0.2, 1.8) #for UTR screening
        ax.set_ylim(-0.002, 0.028) # for target screening
        ...
        ax.hist(df_probe[CENTERED_SCORE_COL].dropna(), bins=centered_bin_edges, density=True, alpha=0.7)
        ax.axvline(0, color="black", linewidth=1)
        ax.set_title("Cell median-divided")
        ax.set_xlabel(CENTERED_SCORE_COL)
        ax.set_ylabel("density")

        ax = fig.add_subplot(gs[p, 4])
        centered_values = df_probe[CENTERED_SCORE_COL].dropna().values
        hist, bin_edges = np.histogram(centered_values, bins=centered_bin_edges, density=True)
        hist_max = hist.max()
        hist_normalized = hist / hist_max if hist_max > 0 else hist
        bin_centers = 0.5 * (bin_edges[1:] + bin_edges[:-1])
        ax.bar(bin_centers, hist_normalized, width=centered_bin_width, color="#4C78A8", edgecolor="#1F3B5C", linewidth=0.25, alpha=0.65)
        ...
        ax.set_xlabel("cell-median-divided score")
        ...
        ax.set_ylabel("count")
        #ax.set_xlim(-10, 80) # for UTR screening
        ax.set_xlim(-6, 40) # for target screening
    ...
    top25_hotspot_df = select_top_fraction_by_probe(summary_df, "hotspot_index", fraction=0.25)
    plot_metric_bar(fig, gs, top25_hotspot_df, "hotspot_index", (row, slice(8, 10)), "Top 25% hot spot index", "score")
...
def main():
    ...
    df_all = add_cell_median_division(df_all)

    df_all.to_csv(os.path.join(OUTPUT_ROOT, "_all_data_before_global_scoring_cell_median_division.csv"), index=False)
    df_cell.to_csv(os.path.join(OUTPUT_ROOT, "_cell_data_cell_median_division.csv"), index=False)
