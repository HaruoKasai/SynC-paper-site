...
def calculate_as_score(spine_csv, back_csv, cell_count, cond, probe_name):
    ...
    # 体積は平均で正規化
    # if probe_name == 'AS176' and cond == 'APV' and noapv_filler_mean is not None:
    #     X_normalized = sp_minus_back_x_area / (noapv_filler_mean / apv_filler_mean)
    # else:
...
for p, probe in enumerate(probe_list):
    ...
    for spine_csv, back_csv in zip(spine_csv_files, back_csv_files):
        ...
        if "control" in spine_csv.lower():
            ...
            # if os.path.basename(probe) == 'AS176':
            #     noapv_filler_mean = np.mean(X_norm)
            ...
            # if os.path.basename(probe) == 'AS176':
            #     apv_filler_mean = np.mean(X_norm)
...
plot_graph(zscore_combined_df.rename(columns={'z_score': 'index_val'}), position=(6,5), title="Z-score")


hotspot_index_df.to_csv(os.path.join(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data", "_hotspot_index.csv"), index=False)
zscore_combined_df.to_csv(os.path.join(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data", "_z_score.csv"), index=False)
SD_df.to_csv(os.path.join(r"\\DESKTOP-WS2\data\Probe_paper_2023\dissociate\UTR_screening_data", "_standard_deviation.csv"), index=False)
