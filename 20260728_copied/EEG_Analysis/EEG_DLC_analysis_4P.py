...
def rotary_analysis (A, B, Z, exp_duration, time_bin =1, radius = 50, resolution =100): #radius (mm), resolution (P/R)
    ...
    # Create toggling pattern for A and B without a for loop
    ...
    if np.sum(velocities) <= 0:
        velocities = -1 * velocities #順向き方向の修正
    return velocities, time_centers

def plot_binned_timeseries (data, time_centers, ylabel, gs, ax): #time_centers(s)
    print(time_centers)
    ax = fig.add_subplot(gs[ax])
    ax.grid(False)
    ax.plot(time_centers, data, lw=1)
    xticks = np.arange(0, time_centers[-1], 1)
    # xtick_labels = np.arange(0, int(time_centers[-1] / 60), 0.5)
    xtick_labels = np.arange(0, time_centers[-1], 1)
    ax.set_xticks(xticks)
    ax.set_xticklabels(xtick_labels)
    ax.set_xlabel("min")
...
for e, exp in enumerate(exp_list):
    ...
    # file_path = glob.glob(os.path.join(exp, "*.nev"))[0]
    ...
    # analog_input = [seg.analogsignals[1] for seg in block.segments]
    # ainp1 = analog_input[0][:, 0]
    ...
    # digital_input_port のデータを確認
    if digital_input_data is not None:
        # print("Times:", digital_input_data.times)  # 時間情報
        # print("Labels:", digital_input_data.labels)  # イベントラベル

        times = digital_input_data.times.rescale('s').magnitude  # 秒単位のタイムスタンプ
        labels = digital_input_data.labels.astype(int)  # ラベルを文字列から整数に変換
        num_channels = 16  # チャネル数（通常16または32）
        bitwise_states = np.array([[(label >> i) & 1 for i in range(num_channels)] for label in labels])
        channel_times = {f'channel_{i}': [] for i in range(num_channels)}   # 各チャネルの状態変化のタイムスタンプを取得
        for i in range(num_channels):
            changes = np.where(np.diff(bitwise_states[:, i]) != 0)[0] + 1
            # changes = np.where(bitwise_states[:, i] != 0)[0]
            channel_times[f'channel_{i}'] = times[changes]-start_time
        # 各チャネルの状態変化を出力
        # for channel, t in channel_times.items():
            # print(f"{channel}: {t}")
            # print(len(t))
    else:
        print("No digital_input_port data found.")
    ...
    if digital_input_data is not None:
        ...
        velocities, time_centers = rotary_analysis(A, B, Z, exp_duration=exp_duration)
        plot_binned_timeseries(velocities, time_centers, ylabel = "Velocity (mm/s)", gs=gs, ax=(8, slice(0, 3)))
    ...
    fig.savefig(os.path.join(dir, mouse_name+"_"+exp_name+"_timeseries.pdf"), dpi=150, transparent=True)