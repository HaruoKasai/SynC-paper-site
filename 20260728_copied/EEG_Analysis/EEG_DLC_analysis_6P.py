...
mouse_dir = filedialog.askdirectory(initialdir=r"X:\Behavior")
...
for e, exp in enumerate(exp_list):
    ...
    if dlc_type =="Pupillometry":
        ...
        print(dlc_exp_dir)
        csv = glob.glob(os.path.join(dlc_exp_dir,"raw_video","*.csv"))[1] #filtered
        df = pd.read_csv(csv)
        num_points = 8
        ...
        frames_extracted_by_velocity = DLCAnal.time_series_velocity(df, real_frame_time=real_frame_time,
                                                                    arena_mm_per_pix=arena_mm_per_pix,
                                                                    exp_duration=exp_duration, fig=fig,
                                                                    ax=(8, slice(0, 3)), gs=gs, body_part="body_center",
        ...
        for v in range(len(frames_extracted_by_velocity)):
            ...
            event_df = DLCAnal.frame_to_sec(frames_extracted_by_velocity[v], real_frame_time, event_df, event_name,
                                            tolerable_frame_drop=0, min_duration=3, exp_duration=exp_duration)
        ...
        frame_approaching, frame_leaving = DLCAnal.time_series_distance_to_object(df, object_coordinate=coordinate,
                                                                                  real_frame_time=real_frame_time,
                                                                                  arena_mm_per_pix=arena_mm_per_pix,
                                                                                  exp_duration=exp_duration, fig=fig,
                                                                                  ax=(9, slice(0, 3)), gs=gs,
                                                                                  body_part="snout",
        ...
        event_df = DLCAnal.frame_to_sec_v2(frame_approaching, real_frame_time, event_df,
                                           event_name="Before approaching(-10 ~ -5 sec)", before_sec=-10, after_sec=-5,
                                           exp_duration=exp_duration)
        event_df = DLCAnal.frame_to_sec_v2(frame_approaching, real_frame_time, event_df,
                                           event_name="0-5 sec after approach", before_sec=0, after_sec=5,
                                           exp_duration=exp_duration)
        event_df = DLCAnal.frame_to_sec_v2(frame_approaching, real_frame_time, event_df,
                                           event_name="-5 ~ 15 sec after approach)", before_sec=-5, after_sec=15,
                                           exp_duration=exp_duration)
        ...
        event_df = DLCAnal.frame_to_sec_v2(frame_leaving, real_frame_time, event_df, event_name="Left (5-10 sec)",
                                           before_sec=5, after_sec=10, exp_duration=exp_duration)
        ...
        frame_approaching_c, frame_leaving_c = DLCAnal.time_series_distance_to_object(df,
                                                                                      object_coordinate=coordinate_ctrl,
                                                                                      real_frame_time=real_frame_time,
                                                                                      arena_mm_per_pix=arena_mm_per_pix,
                                                                                      exp_duration=exp_duration,
                                                                                      fig=fig, ax=(9, slice(0, 3)),
                                                                                      gs=gs, body_part="snout",
                                                                                      distance_to_boundary_mm=100,
                                                                                      plot=False)
    ...
    plt.plot(areas, label="Ellipse Area", color='b')
    plt.xlabel("Time Step")
    plt.ylabel("Ellipse Area (pixels²)")
    plt.title("Ellipse Area Over Time")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    fig.savefig(os.path.join(dir, mouse_name+"_"+exp_name+"_EEG_timeseries.pdf"), dpi=150, transparent=True)