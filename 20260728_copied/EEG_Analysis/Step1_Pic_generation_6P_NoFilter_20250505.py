...
def combined_spectrum_image(signal, sampling_rate, time_point, freq_max, freq_min, output_dir):
    ...
    output_dir = os.path.join(output_dir, "EEG_Raw_Trace")
    ...
    for time_point in time_points:
        ...
        spectrum_graph(m1_signal, sampling_rate, [time_point], freq_max, freq_min, ax)
        ax_lfp = fig.add_subplot(gs[0, 1])
        spectrum_graph(m1v1, sampling_rate, [time_point], freq_max, freq_min, ax_lfp)

        # 绘制0-50Hz滤波后的信号
        ax_m1 = fig.add_subplot(gs[3, 0])
        plot_signal(ax_m1, m1_signal, sampling_rate, "M1-Ce", 500, [time_point])
        ax_m1v1 = fig.add_subplot(gs[3, 1])
        plot_signal(ax_m1v1, m1v1, sampling_rate, "M1-V1", 500, [time_point])

        # 绘制EMG信号
        emg = (ch3_signal - ch4_signal).magnitude.flatten()
        ax_emg1 = fig.add_subplot(gs[4, 0])
        ax_emg2 = fig.add_subplot(gs[4, 1])
        plot_signal(ax_emg1, emg, sampling_rate, "EMG", 500, [time_point])
        plot_signal(ax_emg2, emg, sampling_rate, "EMG", 500, [time_point])

        # heatmap 0-100
        ax_db_m1 = fig.add_subplot(gs[1, 0])
        dB_heatmap(m1_signal, sampling_rate, time_point, 100, freq_min, ax=ax_db_m1, title = 'M1_EEG_Power_dB')
        ax_db_m1v1 = fig.add_subplot(gs[1, 1])
        dB_heatmap(m1v1, sampling_rate, time_point, 100, freq_min, ax=ax_db_m1v1, title = 'M1-V1_EEG_Power_dB')

        # heatmap 0-20
        ax_db_20 = fig.add_subplot(gs[2, 0])
        dB_heatmap(m1_signal, sampling_rate, time_point, 20, 1, ax=ax_db_20, title='V_EEG_Power_dB')
        ax_db_lfp_20 = fig.add_subplot(gs[2, 1])
        dB_heatmap(m1v1, sampling_rate, time_point, 20, 1, ax=ax_db_lfp_20, title='M_EEG_Power_dB')

        # 保存图像到新创建的文件夹中
        combined_image_path = os.path.join(output_dir, f"EEG_Raw_Trace_{time_point}.png")
...
if mouse_dir:
    ...
    for root_dir, sub_dirs, files in os.walk(mouse_dir):
        ...
        for ns3_file in ns3_files:
            ...
            combined_spectrum_image(m1_signal, sampling_rate, 0, freq_max, freq_min, output_dir)
else:
