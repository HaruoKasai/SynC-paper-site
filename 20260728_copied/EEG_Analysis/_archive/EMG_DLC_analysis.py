...
for e, exp in enumerate(exp_list):
    ...
    # EEG (Ch1)
    raw_signal = raw_signals[0][:, 1] # 解析channelをCh2に変更した
    ...
    """
    # ch2-ch3　Spectrum　用
    raw_signal_ch2 = raw_signals[0][:, 1]
    raw_signal_ch3 = raw_signals[0][:, 2]
    sampling_rate = int(raw_signal_ch2.sampling_rate.magnitude)
    signal = (raw_signal_ch2.magnitude - raw_signal_ch3.magnitude).flatten()
    exp_duration = len(signal)/sampling_rate #sec"""
    ...
    fig.savefig(os.path.join(dir, mouse_name+"_"+exp_name+"_Ch2-Ch3_timeseries.pdf"), dpi=150, transparent=True)