...
def calculate_spectrum(eeg, sampling_rate):
    ...
    power_spectrum[1:half-1] *= 2  # DCとNyquistを除き2倍
    freqs = np.fft.fftfreq(n, d=1 / sampling_rate)[:half]
...
def process_file(file_path, index, dlc_type,dlc_dir, EEG_ch_dict, EMG_ch_dict, analog_dict, cont_time,result_list, lock):
    ...
    f_stft, t_stft, Zxx = stft(
        eeg, fs=sampling_rate, window=win, nperseg=nperseg, noverlap=noverlap,
        detrend=False, boundary=None, padded=False, return_onesided=True
    )
    ...
    Sxx = np.abs(Zxx) ** 2
    # calculate_spectrum と同じく nperseg で割るだけ（fs では割らない）
    linear_power = Sxx / nperseg  # μV²

    # 片側化の2倍係数（>0Hzを×2, DCとNyquistは×1）
    one_sided_factor = np.ones_like(f_stft)
    one_sided_factor[f_stft > 0] = 2.0
    if nperseg % 2 == 0:
        nyq = sampling_rate / 2.0
        one_sided_factor[np.isclose(f_stft, nyq)] = 1.0
    linear_power *= one_sided_factor[None, :, None]
