...
gs = gridspec.GridSpec(6, 3, height_ratios=[2, 0.4, 1, 1, 1, 0.4])
...
# ns2ファイルのパス
file_path = r"\\DESKTOP-WS2\data\Zhou\Behavior\EEG\Ctrl_mouse\20240820_z155-2(10w)_Pup-Ctrl\Test2_object1-put-in\z155-2(10w)_TEST2_005.ns3"
...
def calculate_spectrum(signal, sampling_rate):
    ...
    # ZHOU add 转换功率谱到dB格式 (From spectrum to dB (10*log10))
...
def heatmap(signal, sampling_rate, time_bin, freq_max, freq_min, dir, gs, ax, iv=[0.5, 4], rep="Power(μV2)"): #iv: どの範囲のpowerでvmaxを決めるか
    ...
    im = ax.imshow(df.T, aspect='auto', cmap='rainbow', origin='lower', vmin=0, vmax=vmax)
...
# ZHOU add db_heatmap by chatgpt-4o
def dB_heatmap(signal, sampling_rate, time_bin, freq_max, freq_min, fig, gs, ax, iv=[0.5, 4]):
    df = pd.DataFrame()
    ...
    for t in range(time_bin_num):
        ...
        bins = np.linspace(freqs.min(), freqs.max(), num_bins + 1)
        bin_indices = np.digitize(freqs, bins)
        power_spectrum_means_db = [power_spectrum_db[bin_indices == i].mean() for i in range(1, num_bins + 1)]
    ...
    # 使用指定的 ax 绘制 dB 热图
    ax_db = fig.add_subplot(gs[(4, slice(0, 3))])
    vmax_db = np.percentile(df.values, 99)  # 选择 99% 分位数作为 vmax 以避免颜色映射溢出

# In this line can change the scale for heat map
    im_db = ax_db.imshow(df.T, aspect='auto', cmap='rainbow', origin='lower', vmin=30, vmax=vmax_db)

    ax_db.set_title('Power Spectrum in dB')
    ax_db.set_xlabel("sec")
    ax_db.set_ylabel("Hz")
    ...
    # 设置 x 轴和 y 轴的刻度值
    xticks = np.arange(0, int(60 * 60 / time_bin) + 1, int(1 * 60 / time_bin))
    ax_db.set_xticks(xticks)
    xtick_labels = np.arange(0, 60 * 60 + 1, 60)
    ax_db.set_xticklabels(xtick_labels)

    yticks = np.arange(0, (freq_max / freq_bin) + 1, int(freq_max / freq_bin / 5))
    ytick_labels = np.arange(freq_min, freq_max + 1, int(freq_max / 5))
    ax_db.set_yticks(yticks)
    ax_db.set_yticklabels(ytick_labels)
...
dB_heatmap(signal, sampling_rate, time_bin, freq_max, freq_min, fig, gs, ax=(4, slice(0, 3)))
...
plot_raw(emg, sampling_rate, ylabel="EMG", ylim=300, gs=gs, ax=(5, slice(0, 3)))
