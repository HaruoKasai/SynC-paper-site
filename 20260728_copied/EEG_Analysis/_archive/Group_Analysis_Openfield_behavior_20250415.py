...
def calculate_center_time(dlc_dir,contime, data_list):
    time_blocks = [[-30,0],[60,90]]
    ...
    in_center_array = np.array(in_center_array_list, dtype=np.int32)
...
def process_group(json_path, group_dict, electrode_dict, state_dict):
    ...
    for group in group_dict:
        ...
        for subgroup in subgroup_list:
            exp_list = glob.glob(os.path.join(dir,subgroup, "20*"))
            for exp in exp_list:
                #TODO if
                ...
                if dlc_dir is not None:
                    data_list = calculate_center_time(dlc_dir, contime, data_list)
                else:
                    print("No dlc data")
...
def main():
    ...
    json_path =r"X:\Behavior\Openfield_EEG\_Group_Analysis_Behavior\_group_analysis_behavior_0415.json"
