...
def plot_PETH(valid_t,time_array, t_pre,t_post,OF_tp,f_stft, t_stft, time_bin_emg, time_bin_power,sampling_rate,eeg, emg, velocity, linear_power, analog_tp, table_tp,
              fig, gs, column, output_dir, ch_name, epoch_name, epoch_type):
    print(epoch_name)
    print(epoch_type)
    ...
    n_event = len(valid_t)
    velocity_array = np.empty([n_event, int((t_post - t_pre) / (OF_tp[1] - OF_tp[0]))])
    ...
    # plot average
    average_power =np.mean(linear_power_array, axis=0)
    ...
    average_v = np.mean(velocity_array, axis=0)
    plot_timeseries(t_OF_tp, average_v, 1, ax0, color='#1f77b4', lw=3, title="Velocity",ylabel="mm/s", ylim=(0, 50), label=None)
    average_emg_rms = np.mean(emg_rms_array, axis=0)
    plot_timeseries(t_emg_tp, average_emg_rms, 1, ax1, color='#1f77b4', lw=3, title="EMG-RMS",ylabel=None, ylim=(0, 200), label=None)


    delta_linear=np.mean(delta_array, axis=0)
    theta_linear=np.mean(theta_array, axis=0)
    alpha_linear=np.mean(alpha_array, axis=0)
    beta_linear=np.mean(beta_array, axis=0)
    gamma_linear=np.mean(gamma_array, axis=0)
    ...
    h5_name = os.path.join(output_dir, epoch_name+"_"+epoch_type+"_"+str(t_pre)+"s_"+str(t_post)+"s_PETH_average_"+ch_name+".h5")
...
def process_events( h5_path, analog_tp, eegs, t_stft, f_stft, linear_powers,dB_power,emg, sampling_rate, breathe, tem, breathing_rate, pupil_size, pupil_tp,
    ...
    for ch in range (len(eegs)):
        ...
        for PETH_time in PETH_time_list:
            ...
            for event_df in [manual_event]: #[event_df, manual_event]:
                for event_name in event_df["event_name"].unique().tolist():
                    fig = plt.figure(figsize=(len(time_blocks)*8, 17))
                    gs = gridspec.GridSpec(8, len(time_blocks)*2, height_ratios=[1, 1, 1, 1,1,1,1,1])
                    plt.subplots_adjust(hspace=0.5)

                    df = event_df.loc[event_df["event_name"] == event_name]
                    start_time_array = df["start_time"].to_numpy()
                    end_time_array = df["end_time"].to_numpy()

                    for b in range (len(time_blocks)):
                        # epoch start ###########
                        mask = (start_time_array >= time_blocks[b][0]*60) & (start_time_array < time_blocks[b][1]*60)
                        start_array = start_time_array[mask] #TODO あとからmaskかける形式の法がいい
                        end_array = end_time_array[mask]
                        valid_t = [t for t in range(len(start_array))
                            if
                            ((t == 0 or end_array[t - 1] - start_array[t] < t_pre ) and end_array[t] - start_array[t] > t_post)
                            and not any(
                                t_pre + start_array[t] <= value*60 <= t_post + start_array[t]
                                for sublist in contime for value in sublist                             )]

                        plot_PETH(valid_t, start_array, t_pre, t_post, OF_tp, f_stft, t_stft, time_bin_emg, time_bin_power,
                                      sampling_rate, eeg, emg, velocity, linear_power, analog_tp, table_tp,
                                      fig, gs, b, output_dir,ch_name,event_name, "start")

                        # epoch end ##########
                        mask = (end_time_array >= time_blocks[b][0] * 60) & (end_time_array < time_blocks[b][1] * 60)
                        start_array = start_time_array[mask]
                        end_array = end_time_array[mask]
                        valid_t = [t for t in range(len(start_array))
                                   if
                                   ((t == len(end_array)-1 or start_array[t+1] - end_array[t] > t_post )and start_array[t]-end_array[t]<t_pre)
                                   and not any(
                                       t_pre + end_array[t] <= value * 60 <= t_post + end_array[t]
                                       for sublist in contime for value in sublist)]

                        plot_PETH(valid_t, end_array, t_pre, t_post, OF_tp, f_stft, t_stft, time_bin_emg, time_bin_power,
                                  sampling_rate, eeg, emg, velocity, linear_power, analog_tp, table_tp,
                                  fig, gs,len(time_blocks)+b, output_dir, ch_name,event_name,  "end")

                    plt.tight_layout()

                    pdf_path = os.path.join (os.path.dirname(h5_path), "PETH_"+event_name+"_"+str(t_pre)+"s_"+str(t_post)+"s___"+ch_name+".pdf")
