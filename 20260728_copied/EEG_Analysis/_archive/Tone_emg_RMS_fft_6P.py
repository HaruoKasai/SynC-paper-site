...
def extract_emg_data(file_path, change_threshold=1000):  # 设置变化阈值
    try:
        ...
        # 返回提取的其他数据
        ch1_signal = raw_signals[0][:, 0]  # LFP 数据
        ch2_signal = raw_signals[0][:, 1]  # EEG 数据
        ch3_signal, ch4_signal = raw_signals[0][:, 2], raw_signals[0][:, 3]  # EMG 数据
        emg = (ch3_signal - ch4_signal).magnitude.flatten()
        eeg = ch2_signal.magnitude.flatten()
        lfp = ch1_signal.magnitude.flatten()

        return emg, eeg, lfp, sampling_rate, time_list_csv_path
...
def plot_combined_rms_and_spectrum(time_points_list, rms_values_list, delta_rms_values_list, labels, freqs, power_spectrums, power_spectrums_lfp, time_points, output_pdf):
    ...
    gs = gridspec.GridSpec(2 + n_plots, 2, height_ratios=[1]*2 + [1]*n_plots)
    ...
    for i, center_time in enumerate(time_points):
        if i < len(power_spectrums):
            ...
            img = ax3.imshow(10 * np.log10(power_spectrums[i]),
                aspect='auto',
                origin='lower',
    ...
    # LFP 频谱热图 (右侧)
    for i, center_time in enumerate(time_points):
        if i < len(power_spectrums_lfp):
            ax_lfp = fig.add_subplot(gs[2 + i, 1])
            img_lfp = ax_lfp.imshow(10 * np.log10(power_spectrums_lfp[i]),
                aspect='auto',
                origin='lower',
                extent=[time_points[0], time_points[-1], freqs[0], freqs[-1]],
                cmap='turbo',
                vmin=30,
                vmax=60)
            ax_lfp.set_title(f'LFP Spectrum at {center_time}s')
            ax_lfp.set_xlabel('Time (s)')
            ax_lfp.set_ylabel('Frequency (Hz)')
            # ax_lfp.set_yscale('log')
            ax_lfp.set_ylim(1, 100)
...
def process_file(file_path, time_points, window_pre=15, window_post=20, rms_window_size=0.5, spectrum_window_size=0.5, freq_bin=0.5):
    emg, eeg, lfp, sampling_rate, _ = extract_emg_data(file_path)
    ...
    for center_time in time_points:
        ...
        for idx in range(spectrum_start_idx, spectrum_end_idx - window_samples + 1, window_samples):
            # 获取 freqs 和 power_spectrum
            if freqs is None:
                freqs, power_spectrum = calculate_spectrum(eeg[idx:idx + window_samples], sampling_rate, freq_bin=freq_bin)
            else:
                _, power_spectrum = calculate_spectrum(eeg[idx:idx + window_samples], sampling_rate, freq_bin=freq_bin)
            spectrum.append(power_spectrum)
            _, power_spectrum_lfp = calculate_spectrum(lfp[idx:idx + window_samples], sampling_rate, freq_bin=freq_bin)
            spectrum_lfp.append(power_spectrum_lfp)

        power_spectrums.append(np.array(spectrum).T if spectrum else None)
        power_spectrums_lfp.append(np.array(spectrum_lfp).T if spectrum_lfp else None)
    ...
    plot_combined_rms_and_spectrum(
        time_points_list, rms_values_list, delta_rms_values_list, labels, freqs, power_spectrums,power_spectrums_lfp,
...
def main():
    ...
    for root_dir, _, files in os.walk(folder_path):
        ...
        for ns3_file in ns3_files:
            ...
            emg, eeg, lfp, sampling_rate, time_list_csv_path = extract_emg_data(file_path)
            if emg is None or eeg is None or sampling_rate is None:
