...
def plot_PETH_v2(start_array, end_array, t_pre, t_post, OF_tp, f_stft, t_stft, time_bin_emg, time_bin_power, sampling_rate,
              eeg, emg, velocity, linear_power, analog_tp, table_tp,
              fig, gs, column, output_dir, ch_name, epoch_name, epoch_type, time_type):
    print(epoch_name)
    print(epoch_type)
    axes = [fig.add_subplot(gs[i, column]) for i in range(8)]
    ax0, ax1, ax2, ax3, ax4, ax5, ax6, ax7 = axes

    n_event = len(start_array)
    ...
    for t in valid_t:
        ...
        """
        # TODO t_velocity, t_emg, t_linear_power, delta, theta, alpha, beta, gamma
    ...
    plot_timeseries(t_OF_tp, average_v, 1, ax0, color='#1f77b4', lw=3, title="Velocity", ylabel="mm/s", ylim=(0, 50),
                    label=None)
    average_emg_rms = np.nanmean(emg_rms_array, axis=0)
    plot_timeseries(t_emg_tp, average_emg_rms, 1, ax1, color='#1f77b4', lw=3, title="EMG-RMS", ylabel=None,
                    ylim=(0, 200), label=None)
    ...
    plot_timeseries(power_time_array, delta, 1, ax3, color='#1f77b4', lw=4, title="delta power", ylabel="(dB)",
                    ylim=(55, 85), label=None)
    plot_timeseries(power_time_array, theta, 1, ax4, color='#1f77b4', lw=4, title="theta power", ylabel="(dB)",
                    ylim=(55, 85), label=None)
    plot_timeseries(power_time_array, alpha, 1, ax5, color='#1f77b4', lw=4, title="alpha power", ylabel="(dB)",
                    ylim=(55, 85), label=None)
    plot_timeseries(power_time_array, beta, 1, ax6, color='#1f77b4', lw=4, title="beta power", ylabel="(dB)",
                    ylim=(55, 85), label=None)
    plot_timeseries(power_time_array, gamma, 1, ax7, color='#1f77b4', lw=4, title="gamma power", ylabel="(dB)",
                    ylim=(55, 85), label=None)
    ...
    h5_name = os.path.join(output_dir, epoch_name + "_" + epoch_type + "_" + str(t_pre) + "s_" + str(
        t_post) + "s_PETH_average_" + time_type + "_" + ch_name + ".h5")
        ...
        for p, PETH_time in enumerate(PETH_time_list):
            ...
            for event_df in [manual_event]: #[event_df, manual_event]:
                ...
                for event_name in event_df["event_name"].unique().tolist():
                    ...
                    for b in range (len(time_blocks)): #この機能いらなさそう
                        if time_type=="rough" or "strict":
                            if time_type=="rough":
                                start_valid_t = [t for t in range(len(start_array))
                                    if
                            ...
                            plot_PETH(start_valid_t, start_array, t_pre, t_post, OF_tp, f_stft, t_stft, time_bin_emg,
                                      time_bin_power,
                                      sampling_rate, eeg, emg, velocity, linear_power, analog_tp, table_tp,
                                      fig, gs, b, output_dir, ch_name, event_name, "start", time_type)

                            plot_PETH(end_valid_t, end_array, t_pre, t_post, OF_tp, f_stft, t_stft, time_bin_emg,
                                      time_bin_power,
                                      sampling_rate, eeg, emg, velocity, linear_power, analog_tp, table_tp,
                                      fig, gs, len(time_blocks) + b, output_dir, ch_name, event_name, "end", time_type)
