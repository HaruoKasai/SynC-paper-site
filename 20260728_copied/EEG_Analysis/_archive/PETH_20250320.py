...
import numpy as np
import pandas as pd
from _archive.EEG_Analysis import plot_timeseries_power, plot_heatmap, plot_timeseries, extract_params
import os
...
def plot_PETH(valid_t,time_array,time_array2, t_pre,t_post,OF_tp,f_stft, t_stft, time_bin_emg, time_bin_power,sampling_rate,eeg, emg, velocity, linear_power, analog_tp, table_tp,
    ...
    print("here")
    print(pupil_tp.shape)
    print(pupil_size.shape)
    print(breathing_rate.shape)
    print(analog_tp.shape)
    ...
    n_event = len(valid_t)
    velocity_array = np.zeros([n_event, int((t_post - t_pre) / (OF_tp[1] - OF_tp[0]))], dtype=np.float64) #TODO
    ...
    t_OF_tp = np.arange(t_pre, t_post, OF_tp[1] - OF_tp[0]) #TODO
    i = 0
    print(t_pre)
    print(valid_t)
    for t in valid_t:
...
def process_events( h5_path, analog_tp, eegs, t_stft, f_stft, linear_powers,dB_power,emg, sampling_rate, breathe, tem, breathing_rate, pupil_size, pupil_tp,
            table_v, table_tp, OF_tp, velocity,cum_d,distance,event_df, EEG_ch_dict, manual_event, contime, dlc_type):
                ...
                for event_name in event_df["event_name"].unique().tolist():
                    ...
                    for b in range (len(time_blocks)):
                        ...
                        plot_PETH(start_valid_t, start_array, end_array, t_pre, t_post, OF_tp, f_stft, t_stft, time_bin_emg,
                                  time_bin_power,
                                  sampling_rate, eeg, emg, velocity, linear_power, analog_tp, table_tp,pupil_tp, pupil_size, breathing_rate, tem,
...
def main():
    ...
    data_folder = r"X:\Behavior\Turntable_EEG\02_Pup-IRES\20250220_z199-4_Pup-IRES-5x-2p-Parietal(18w-MV-Ce-male-6P)_Turntable"  # for development
