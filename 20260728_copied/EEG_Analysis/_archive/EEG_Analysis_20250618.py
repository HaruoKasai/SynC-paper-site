...
def plot_timeseries(tp, data, window_size, ax, color, lw, title, ylabel, ylim, label=None):
    """
    tp, data: numpy
    window_size: int
    """
    ...
    ax.set_ylim(ylim)
    ax.plot(ave_tp, ave_data,lw=lw, color=color, label = label)
    ax.set_title(title)
    ax.set_ylabel(ylabel)
    if label:
        ax.legend(loc="upper right")
...
def save_eeg_analysis_results(dlc_type, analog_tp,
    ...
    if dlc_type == "Pupillometry":
        ...
        ax7_2.plot(time, breathing_rate, label="Breathing_rate (BPM)", color="red", linewidth=0.2)
    ...
    for ax in axes:
        # xticks = list(range(int(analog_tp[0]), int(analog_tp[-1]) + 1, 5*60))
        # ax.set_xticks(xticks)
        # ax.set_xticklabels(list(range(int(analog_tp[0]/60), int(analog_tp[-1]/60)+1, 5)))
...
def main():
    ...
    # data_folder = r"X:\Behavior\Openfield_EEG\Ctrl_mouse\20250325_z233_copy" #for development
    # data_folder = r"X:\Behavior\Openfield_EEG\Ctrl_mouse\20241105_z202_temp" #for development
    # data_folder = r"X:\Behavior\Turntable_EEG\01_WT\20250212_temp" #for development
