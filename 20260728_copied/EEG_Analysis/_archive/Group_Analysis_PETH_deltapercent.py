                ...
                for time in PETH_time_list:
                    ...
                    for align_type in alignment_types:
                        ...
                        for e, type in enumerate(epoch_types):
                            ...
                            for h, h5_file in enumerate(h5_files):
                                ...
                                if not any(np.isnan(values).any() for values in data.values()):
                                    plot_timeseries(data["OF_tp"], data["velocity"], velocity_window, ax0, plt.get_cmap("tab10")(h),0.3, None, None, ax0_ylim, None)
                                    plot_timeseries(data["emg_tp"], data["emg_rms"], rms_window, ax1, plt.get_cmap("tab10")(h), 0.3, None, None, (0,250),None)
                                    ...
                                    plot_timeseries(data["power_time_array"], data["gamma"], power_window, ax3, plt.get_cmap("tab10")(h), 0.3, None, None, (-80,80),None)
                                    plot_timeseries(data["power_time_array"], data["delta"], power_window, ax4, plt.get_cmap("tab10")(h), 0.3, None, None, (-80,320),None)
                                    plot_timeseries(data["power_time_array"], data["theta"], power_window, ax7, plt.get_cmap("tab10")(h), 0.3, None, None, (-100,200),exp_ab)
                                    plot_timeseries(data["power_time_array"], data["alpha"], power_window, ax8, plt.get_cmap("tab10")(h), 0.3, None, None, (-100,200),None)
                                    plot_timeseries(data["power_time_array"], data["beta"], power_window, ax9, plt.get_cmap("tab10")(h), 0.3, None, None, (-100,200),None)
                        ...
                        if plot:
                            ...
                            pdf_path = os.path.join(dir, "_Group_Analysis_EEG-EMG_PETH", align_type+"_"+group+"_"+state+"_"+str(time[0])+"s_"+str(time[1])+"s_PETH_"+electrode_name+".pdf")
...
def main():
    ...
    json_path =r"X:\Behavior\Openfield_EEG\_Group_Analysis_EEG-EMG_PETH\_group_analysis_param_PETH_0415.json"
