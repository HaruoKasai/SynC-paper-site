import pandas as pd
from EEG_Analysis import plot_timeseries
import os
import glob
...
#TODO timeseriesのvelocityは不要になったので消す
...
def process_group(json_path, group_dict,state_dict):
    ...
    for g, (group, exp_list) in enumerate(group_dict.items()):
        ...
        for h, h5_file in enumerate(h5_files):
            ...
            plot_timeseries(bin_tp, bin_v_active, 1, ax0, plt.get_cmap("tab20")(h), 0.25, "", None, (0, 80), None)
            print(bin_v_active)
        ...
        for e, event_file in enumerate (event_files):
            ...
            label = os.path.basename(os.path.dirname(os.path.dirname(event_file)))[:15]
            plot_timeseries(ratio_df["bin_center"].values/60, ratio_df["StateC_%"].values, 1, ax1, plt.get_cmap("tab20")(e), 0.25, None, None, (-5,105),label)
        ...
        sem_active = np.nanstd(group_active, axis=0, ddof=1) / np.sqrt(np.sum(~np.isnan(group_active), axis=0))
        sem_NREM = np.nanstd(group_NREM, axis=0,ddof=1) / np.sqrt(np.sum(~np.isnan(group_NREM),axis=0))
        sem_stateC = np.nanstd(group_stateC, axis=0,ddof=1) / np.sqrt(np.sum(~np.isnan(group_stateC),axis=0))

        plot_timeseries(ratio_df["bin_center"].values/60, mean_active, 1, ax1, "blue", 2.5, "State", "%", (-5, 105), "Motive")
        plot_shade(ratio_df["bin_center"].values/60, mean_active, sem_active, 1, ax1, "blue", alpha=0.3)
        plot_timeseries(ratio_df["bin_center"].values / 60, mean_NREM, 1, ax1, "green", 2.5, "State", "%",
                        (-5, 105), "SWS")
        plot_shade(ratio_df["bin_center"].values / 60, mean_NREM, sem_NREM, 1, ax1, "green", alpha=0.3)
        plot_timeseries(ratio_df["bin_center"].values / 60, mean_stateC, 1, ax1, "red", 2.5, "State", "%",
                        (-5, 105), "StateC")
        plot_shade(ratio_df["bin_center"].values / 60, mean_stateC, sem_stateC, 1, ax1, "red", alpha=0.3)
        ...
        # =========================================================
        # ✅ 保存各个个体（individual）的数据到同一组CSV
        # =========================================================
        save_dir = os.path.join(dir, "_Group_Analysis_timeseries")
        os.makedirs(save_dir, exist_ok=True)

        time_min = ratio_df["bin_center"].values / 60

        # 各状态的 DataFrame
        df_active = pd.DataFrame(group_active).T
        df_NREM = pd.DataFrame(group_NREM).T
        df_stateC = pd.DataFrame(group_stateC).T

        # 设置列名（如 Ind1, Ind2, ...）
        df_active.columns = [f"Ind{i + 1}_Active_%" for i in range(df_active.shape[1])]
        df_NREM.columns = [f"Ind{i + 1}_NREM_%" for i in range(df_NREM.shape[1])]
        df_stateC.columns = [f"Ind{i + 1}_StateC_%" for i in range(df_stateC.shape[1])]

        # 添加时间列
        df_all = pd.concat([pd.Series(time_min, name="Time_min"),
                            df_active, df_NREM, df_stateC], axis=1)

        # 添加 group 平均和SEM
        df_all["Mean_Active_%"] = mean_active
        df_all["SEM_Active_%"] = sem_active
        df_all["Mean_NREM_%"] = mean_NREM
        df_all["SEM_NREM_%"] = sem_NREM
        df_all["Mean_StateC_%"] = mean_stateC
        df_all["SEM_StateC_%"] = sem_stateC

        csv_path = os.path.join(save_dir, f"{group}_timeseries_individual.csv")
        df_all.to_csv(csv_path, index=False, encoding="utf-8-sig")
        print(f"✅ Saved group CSV: {csv_path}")
        # =========================================================
...
def main():
    ...
    json_path = r"X:\Behavior\Openfield_EEG\_Group_Analysis_timeseries\_group_analysis_param_stateC_timeseries.json"
