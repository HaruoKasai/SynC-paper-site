...
# ns2文件路径
# file_path = r"\\DESKTOP-WS2\data\Zhou\Behavior\EEG\SYNCit-C_ROI\20240925_z174_SYNCit-C-IRES-ROI(10w-M1-Ce)\04-O2-10min_004\04-O2-10min_004.ns3"
# file_path = r"\\DESKTOP-WS2\data\Zhou\Behavior\EEG\SYNCit-C_Pup\20240925_z178-4_SYNCit-C-IRES-Pup(5w-M1-Ce)\03-rest-30min_003\03-rest-30min_003.ns3"
# file_path = r"\\DESKTOP-WS2\data\Zhou\Behavior\EEG\20240820_z155-2(10w)_Pup-Ctrl\Test2_object1-put-in\z155-2(10w)_TEST2_005.ns3"
...
def combined_spectrum_image(signal, sampling_rate, time_point, freq_max, freq_min, output_dir):
    ...
    output_dir = os.path.join(output_dir, "_EEG_Raw_Trace")# or named as "combined_analysis_video"
    ...
    for time_point in time_points:
        ...
        gs = gridspec.GridSpec(5, 1, height_ratios=[0.4, 0.4,0.2, 0.4, 0.4])
        # 调整子图间隔
        ...
        plot_signal(ax_filtered_9_16, filtered_signal_9_16, sampling_rate, "9-16Hz Filtered Signal", 500, [time_point])"""

        # 绘制0-100Hz滤波后的信号
        """filtered_signal_0_100 = butter_lowpass_filter(signal, 100, sampling_rate)"""
        ax_filtered_0_100 = fig.add_subplot(gs[3, 0])
        plot_signal(ax_filtered_0_100, signal, sampling_rate, "EEG Signal", 500, [time_point])

        # 绘制EMG信号
        emg = (ch2_signal - ch3_signal).magnitude.flatten()
        ax_emg = fig.add_subplot(gs[4, 0])
        ...
        ax_db_20 = fig.add_subplot(gs[2, :])
        dB_heatmap(signal, sampling_rate, time_point, 20, 0, ax=ax_db_20)

        # 保存图像到新创建的文件夹中
        combined_image_path = os.path.join(output_dir, f"EEG_Raw_Trace_{time_point}.png") # or named as "combined_analysis"
